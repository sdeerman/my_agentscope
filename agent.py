...
class DecisionOutput(BaseModel):
    """结构化输出：回合动作决策。"""
    speech: str = Field(description="你的发言内容，必须填写，不能为空")
    vote: Optional[str] = Field(default=None, description="投票目标（白天投票时填写）")
    night_action: Optional[Dict[str, Any]] = Field(default=None, description="夜间行动（狼人/预言家/女巫等角色的夜间动作）")
...
class PlayerAgent(AgentBase, str):
    ...
    def __init__(self, name: str):
        ...
        # 玩家名单管理
        self.all_player_names: List[str] = [f"Player{i+1}" for i in range(9)]
        self.alive_player_names: List[str] = self.all_player_names.copy()
    ...
    async def observe(self, observation: Any) -> None:
        ...
        if event.get("type") in {"game_start", "role_assignment"}:
            ...
            # 重置存活玩家列表
            self.alive_player_names = self.all_player_names.copy()
        
        # 玩家死亡事件：更新存活列表
        if event.get("type") in {"player_eliminated", "player_died", "night_kill", "day_vote_out"}:
            eliminated = event.get("target") or event.get("eliminated") or event.get("voted_out")
            if isinstance(eliminated, str) and eliminated in self.alive_player_names:
                self.alive_player_names.remove(eliminated)
                print(f"[ALIVE_UPDATE {self.name}] {eliminated}已出局，当前存活: {self.alive_player_names}")
        
        # 从Moderator消息中提取死亡信息
        content = event.get("content", "")
        if isinstance(content, str):
            # 匹配"昨晚被淘汰的玩家有：PlayerX"
            import re
            death_match = re.search(r'昨晚被淘汰的玩家有：([^。]+)', content)
            if death_match:
                dead_players_str = death_match.group(1)
                for player in self.all_player_names:
                    if player in dead_players_str and player in self.alive_player_names:
                        self.alive_player_names.remove(player)
                        print(f"[ALIVE_UPDATE {self.name}] {player}已出局（夜间），当前存活: {len(self.alive_player_names)}人")
            
            # 匹配"PlayerX 被淘汰"或"PlayerX, you已被淘汰"
            elim_match = re.search(r'(Player\d+)[,，]?\s*(?:你)?已?被淘汰', content)
            if elim_match:
                player = elim_match.group(1)
                if player in self.alive_player_names:
                    self.alive_player_names.remove(player)
                    print(f"[ALIVE_UPDATE {self.name}] {player}已出局（投票），当前存活: {len(self.alive_player_names)}人")
    ...
    async def reply(
        ...
        This method provides compatibility with AgentScope's ReActAgent interface.
        
        Args:
            msg: The input message(s) to the agent (can be None for狼人杀game).
            structured_model: The required structured output model.
            
        Returns:
            Msg: The output message generated by the agent.
        """
        # Call the internal __call__ method with proper parameters
        return await self.__call__(msg, structured_model=structured_model)
    ...
    async def __call__(self, *args: Any, **kwargs: Any) -> Msg:
        ...
        # ⚠️ 只在LLM真的返回空speech时才使用fallback（理想情况下不应该发生）
        if not speech:
            print(f"[WARN {self.name}] LLM返回空speech，使用fallback生成")
        ...
        try:
            if cls_name == "DiscussionModel":
                # 检查是否达成共识：如果某个目标被提到2次以上，认为达成共识
                wolf_suggest = self._wolf_suggest_top()
                reach_agreement = False
                if wolf_suggest and len(wolf_suggest) > 0:
                    top_count = wolf_suggest[0][1] if len(wolf_suggest[0]) > 1 else 0
                    reach_agreement = (top_count >= 2)  # 至少2个狼人提到同一个目标
                metadata = {"reach_agreement": reach_agreement}
                if (self.role or "").lower() == "wolf":
                    print(f"[WOLF_DISCUSS {self.name}] 共识统计: {wolf_suggest}, 达成共识: {reach_agreement}")
                ...
                if (self.role or "").lower() == "wolf":
                    ...
                    wolf_suggest = self._wolf_suggest_top()
                    print(f"[WOLF_VOTE {self.name}] 讨论共识: {wolf_suggest}, 选择目标: {target}")
                
                # ⚠️ 验证目标是否存活
                if target and target not in self.alive_player_names:
                    print(f"[VOTE_FILTER {self.name}] 目标{target}已出局，重新选择")
                    target = None
                
                # 如果没有共识目标，从怀疑列表中选择【仅限存活玩家】
                if not target and self.learned_stats.get("suspicion"):
                    for name, _ in sorted(self.learned_stats["suspicion"].items(), key=lambda kv: kv[1], reverse=True):
                        if name and name != self.name and name in self.alive_player_names:
                            target = name
                            break
                # 如果还没有目标，从文本中提取【仅限存活玩家】
                if not target:
                    candidates = self._extract_names_from_text(text_str)
                    target = next((n for n in candidates if n != self.name and n in self.alive_player_names), None)
                # 最后的兜底 - 从存活玩家中真随机选择
                if not target:
                    import random
                    available_players = [p for p in self.alive_player_names if p != self.name]
                    if available_players:
                        target = random.choice(available_players)
                        print(f"[VOTE_RANDOM {self.name}] 随机选择存活目标: {target}，存活列表: {available_players}")
                    else:
                        # 如果存活列表为空，降级到all_player_names（不应该发生）
                        all_players = [f"Player{i+1}" for i in range(9)]
                        available_players = [p for p in all_players if p != self.name]
                        target = random.choice(available_players) if available_players else "Player1"
                        print(f"[VOTE_ERROR {self.name}] 存活列表为空！降级选择: {target}")
                # 确保只设置 vote 字段
                ...
                for name, _ in sorted(self.learned_stats.get("suspicion", {}).items(), key=lambda kv: kv[1], reverse=True):
                    if (name and 
                        ...
                        name not in self.seer_checked_players and
                        name in self.alive_player_names):
                        target = name
                        break
                # 如果没有找到，从文本中提取候选人【仅限存活玩家】
                if not target:
                    candidates = self._extract_names_from_text(text_str)
                    target = next((n for n in candidates 
                                  if n != self.name 
                                  and n not in self.wolf_teammates 
                                  and n not in self.seer_checked_players
                                  and n in self.alive_player_names), None)
                # 如果还是没有找到，真随机选择一个未查验的人【仅限存活玩家】
                if not target:
                    # 从存活玩家中找一个未查验的
                    available_candidates = [
                        p for p in self.alive_player_names 
    ...
    def _build_prompt(self, 
        ...
        # 添加存活状态和天数提示
        day_text = f"第{self.day_count + 1}天"
        day_desc = "首夜/第一天" if self.day_count == 0 else f"第{self.day_count + 1}天"
        status_text = f"【当前状态】你是{self.name}，你的角色为{self.role}，你现在{'存活并正在参与游戏' if self.alive else '已经出局'}。现在是游戏的{day_desc}。\n\n"
        ...
        if is_day_discussion:
            # 生成天数相关描述
            if self.day_count == 0:
                day_context = "现在是首夜/第一天，昨晚是第一次狼人刀人，信息较少。"
                death_desc = "昨晚（首夜）死亡的玩家"
            elif self.day_count == 1:
                day_context = "现在是第二天，已经过了一晚。"
                death_desc = "昨晚（第二夜）死亡的玩家"
            elif self.day_count == 2:
                day_context = "现在是第三天，已经过了两晚。"
                death_desc = "昨晚（第三夜）死亡的玩家"
            else:
                day_context = f"现在是第{self.day_count + 1}天，已经过了{self.day_count}晚。"
                death_desc = f"昨晚（第{self.day_count}夜）死亡的玩家"
            ...
            special_instruction = (
                ...
                f"- ❌ 不要说「首夜出局」，要根据实际天数说「{death_desc}」\n"
            ...
            if self.day_count == 0:
                ...
                night_desc = f"第{self.day_count}夜"
                info_hint = f"根据前{self.day_count}天的信息"
            
            special_instruction = (
                f"\n\n【当前场景：狼人夜间讨论 - {night_desc}】\n"
                "⚠️ 这是狼人夜间私密讨论，只有狼人队友能看到，其他玩家看不到！\n"
                "⚠️ 不要说「建议大家关注」「大家要观察」这种白天的话，这是夜间私密讨论！\n"
                "⚠️ 直接和狼人队友商量：「我建议今晚刀PlayerX」「我同意刀PlayerX」\n\n"
                "【你的队友】：" + ", ".join(self.wolf_teammates) + "\n\n"
                "【讨论目标】：决定今晚要刀杀哪个玩家\n"
                "【优先级】：神职（预言家、女巫、猎人） > 可疑村民 > 普通村民\n"
                + teammate_context +
                "\n\n【你的发言格式】：\n"
                f"- {info_hint}：「队友们，{'首夜我们随机选一个吧' if self.day_count == 0 else '根据今天的情况'}，我建议刀PlayerX？」\n"
                "- 有明确信息时：「我建议今晚刀PlayerX，因为他白天的发言很像预言家」\n"
                "- 回应队友：「我同意PlayerY的建议，我也支持刀PlayerX」\n"
                "- 不同意见：「我觉得PlayerX不是好目标，我建议刀PlayerZ，因为...」\n\n"
                "-【绝对禁止】不要说「建议大家」「大家要观察」「我暂时保留」这些白天的话！\n"
                "-【绝对要求】`speech`字段必须填写，直接说「我建议刀PlayerX」或「我同意刀PlayerY」\n"
            )
        elif is_wolf_vote:
            # 提取讨论阶段队友的建议，用于投票参考
            ...
            if wolf_targets_discussed:
                top_targets = sorted(wolf_targets_discussed.items(), key=lambda x: x[1], reverse=True)
                if top_targets:
                    consensus_target = top_targets[0][0]
                    vote_context = f"\n\n【队友讨论共识】：\n"
                    vote_context += f"- 提到最多的目标：{consensus_target} ({top_targets[0][1]}次)\n"
                    if len(top_targets) > 1:
                        vote_context += f"- 次选目标：{top_targets[1][0]} ({top_targets[1][1]}次)\n"
                    vote_context += f"\n⚠️ 为了确保狼人投票一致性，你必须投票给：{consensus_target}\n"
                    vote_context += "⚠️ 不要投票给其他人，否则可能导致票数分散！\n"
            ...
            special_instruction = (
                ...
                "- 在`vote`字段填写你要刀的玩家名字\n"
                "- 在`speech`字段简短说明理由\n"
                "- 例如：「我投" + (consensus_target if consensus_target else "PlayerX") + "，按照刚才的计划」\n"
                "- 或者：「我投" + (consensus_target if consensus_target else "PlayerX") + "，队友都同意了」\n\n"
                "-【绝对禁止】不要投票给讨论中没提到的人！\n"
                "-【绝对要求】speech和vote字段都必须填写！\n"
            )
        elif is_seer_check:
            special_instruction = (
                "\n\n【当前场景：预言家查验】\n"
        ...
        user = (
            ...
            "⚠️【输出要求】⚠️\n"
            "1. 你必须输出标准JSON格式\n"
            "2. `speech`字段是必填的，绝对不能为空！必须写出你的推理和发言\n"
            "3. `vote`字段在需要投票时填写玩家名字\n"
            "4. `night_action`字段在夜间行动时填写\n\n"
            "示例（注意speech必须有实质内容）：\n"
            '{"speech":"根据昨晚的情况，我怀疑Player3，因为他的发言很可疑","vote":"Player3","night_action":null}\n\n'
            "⚠️ 再次强调：speech字段必须填写，不能为空字符串！"
        )
        return f"[SYSTEM]\n{sys}\n[USER]\n{user}"
    ...
    def _gen_decision(self, prompt: str) -> DecisionOutput:
        ...
        try:
            ...
            # JSON 解析失败，返回默认决策
            print(f"[ERROR {self.name}] JSON解析失败: {e}, 原文: {text[:200]}")
            # 尝试使用LLM返回的原文（如果有实质内容）
            if text and len(text) > 20 and len(text) < 500:
                print(f"[FALLBACK {self.name}] 使用LLM原文作为speech")
                return DecisionOutput(speech=text.strip())
            # 最后的兜底fallback
            fallback_speech = self._generate_fallback_speech(text)
            print(f"[FALLBACK {self.name}] 使用生成的fallback: {fallback_speech[:50]}")
            return DecisionOutput(speech=fallback_speech)
    
    def _generate_fallback_speech(self, llm_text: str = "") -> str:
        """当LLM完全失败时的最小兜底fallback（理想情况下不应该频繁触发）"""
        import random
        ...
        # 最简单的兜底发言（不区分角色，因为这是异常情况）
        fallback_speeches = [
            "目前信息还不够充分，我需要更多时间来分析。",
            "我暂时没有明确的判断，希望能听到更多信息。",
            "让我先观察一下其他人的发言再做决定。",
        ]
        ...
        *,
        ...
        if is_day_discussion:
            ...
            if role == "wolf":
                ...
                if suspect_name:
                    if self.day_count == 0:
                        return f"首夜信息较少，但我觉得{suspect_name}有些可疑，建议大家关注。"
                    else:
                        return f"根据{time_ref}的情况和今天的发言，我怀疑{suspect_name}。大家觉得呢？"
        ...
        if is_wolf_discussion:
            ...
            if teammate_suggestions:
                ...
                if self.day_count == 0:
                    ...
                    return f"队友们，首夜我们随机选一个吧，我建议刀{target}，{reason}。"
                else:
                    # 第二夜及以后
                    reasons = [
                        "他今天白天发言很像预言家，一直在引导大家",
                        "他的投票记录和其他人不一致，很可疑",
                        "他今天的发言感觉身份不干净",
                        f"根据前{self.day_count}天的观察，他很可疑",
                    ]
                    reason = random.choice(reasons)
        ...
        if is_wolf_vote:
            ...
            if wolf_targets_discussed:
                top_targets = sorted(wolf_targets_discussed.items(), key=lambda x: x[1], reverse=True)
                if top_targets:
                    target = top_targets[0][0]  # 提到最多的目标
            
            # 如果没有讨论共识，尝试从LLM决策获取
            if not target and hasattr(decision, 'metadata') and isinstance(decision.metadata, dict):
                night_action = decision.metadata.get('night_action', {})
                if isinstance(night_action, dict):
                    target = night_action.get('target')
                if not target:
    ...
    def _wolf_suggest_top(self) -> List[List[Any]]:
        ...
        # 检查所有类型的消息（包括speech, text, wolf_chat等）
        for ev in self._history_events[-30:]:
            spk = ev.get("speaker") or ev.get("name")
            
            # 只统计狼人队友的发言
            if not (isinstance(spk, str) and spk in self.wolf_teammates):
                continue
            
            # 方法1: 从target字段获取
            tgt = ev.get("target")
            if isinstance(tgt, str) and tgt != spk:
            ...
            if isinstance(content, str):
                # 提取所有玩家名
                for p in self.all_player_names:
                    if p in content and p != spk and p not in self.wolf_teammates:
    ...
    def _consensus_wolf_target(self) -> Optional[str]:
        ...
        if top:
            target = str(top[0][0])
            # ⚠️ 验证目标是否存活
            if target in self.alive_player_names:
                return target
            print(f"[WOLF_TARGET {self.name}] 共识目标{target}已出局，重新选择")
        
        # 从怀疑列表中选择【仅限存活玩家】
        suspects = sorted(self.learned_stats.get("suspicion", {}).items(), key=lambda kv: kv[1], reverse=True)
        for name, _ in suspects:
            if name and name != self.name and name not in self.wolf_teammates and name in self.alive_player_names:
                return name
        
        # 如果没有怀疑对象，从存活玩家中真随机选择
        available_targets = [
            p for p in self.alive_player_names 
